...
do
{
    ...
    let menu = Number(prompt("ingrese 1 si quiere jugar \nIngrese 2 si quiere ver su historial\ningrese 3 si quiere salir"))

    if(menu == 1)
    {
        cant_numeros = 0
        while((cant_numeros < 1) || (cant_numeros > 9)) //se elije la cantidad de numeros con la que se quiere jugar
            cant_numeros = Math.floor(prompt("elija la cantidad de numeros con los que quiere jugar, debe ser mayor a 0 y debe ser un numero entero"))
        ...
        do{
            ...
            historial_intentos[cant_intentos] = intento //se agrega un historial de numeros para facilitar la resolucion
            cant_intentos ++
        }
...
function esCorrecto ( numero )
{
    ...
    {
        alert("FELICIDADES! Acertaste al numero!" + "\nY solo te tomo " + cant_intentos +" intentos")
        salida = true
    }
    else
...
function esValido( numero )
{
    ...
    if((seRepite( numero)) || (numero >= Math.pow(10,cant_numeros)) || (numero[0] == 0))
    {
        valido = false
    }
    return valido
...
function mostrarHistorial(partidas)
{
    historial = []
        for(let i = 2; i <= 9; i++)
        {
            ...
            partidas.filter((elm)=> elm.cant_numeros == i).forEach((partida)=> {
            ...
            if(cant_partidas)
            {
                alert(`Jugando con ${i} numeros este es tu historial:
...
function pedirNumero(error)
{
    ...
    if(error)
    {
        intento = prompt(`El numero ingresado no es valido. Por favor, ingrese un numero de ${cant_numeros} digitos (del 1 al 9)`)
